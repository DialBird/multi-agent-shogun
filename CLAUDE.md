...
2. **対応する instructions を読む**:
   ...
   - ashigaru ({PROJECT_ID}-multiagent:0.1-5) → instructions/ashigaru.md
3. **禁止事項を確認してから作業開始**
...
- **完了通知**: 全タスク完了時、家老がmacOS通知を送信
- 理由: 殿（人間）の入力中に割り込みが発生するのを防ぐ

### 完了通知（macOS）
全タスク完了時、家老がmacOS通知で殿に報告：
- **タイトル**: `🏯 {PROJECT_ID}`（例: `🏯 FEEEP-Resort`）
- **本文**: `{cmd_id}: {タスク概要}`（例: `cmd_001: API調査が完了`）
- **サウンド**: 通常は `Glass`、要対応ありは `Ping`
...
### {PROJECT_ID}-multiagent セッション（6ペイン）
- Pane 0: karo（家老）
- Pane 1-5: ashigaru1-5（足軽）
